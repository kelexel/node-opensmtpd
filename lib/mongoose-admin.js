...
var MongooseAdmin = prime({
	 ...
	 * @api public
	 */
	listModelDocuments: function(collectionName, scope, start, count, onReady) {
		scope = scope ? scope : {};
		...
		_model.find(scope).skip(start).limit(count).execFind(function(err, documents) {
			if (err) {
	...
	updateDocument: function(user, collectionName, documentId, params, onReady) {
		...
		var model = this._models[collectionName].model;
		var options = this._models[collectionName].options;
			...
			if (err) {
				...
				for (field in fields) {
					if (params[field]) {
						...
						// if (options.passwordField && options.passwordField.contains(field)) {
				...
				if (document.setPassword) {
					document.password = document.hash;
				}
				this.updateSave(self, user, document, collectionName, onReady)
			}
		}.bind(this));
	},
	updateSave: function(self, user, document, collectionName, callback) {
		document.save(function(err) {
			if (err) {
				logger.warn('Unable to update document: ' + err);
				callback('Unable to update docuemnt', null);
			} else {
				if (self._models[collectionName].options && self._models[collectionName].options.post) {
					document = self._models[collectionName].options.post(document);
				}
